...
public class WaveCannonRenderer implements BlockEntityRenderer<WaveCannonBlockEntity> {
    ...
    public void render(WaveCannonBlockEntity be, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        ...
        // 向きの補正 (Z+方向を正面とする)
        switch (facing) {
            case NORTH: poseStack.mulPose(Axis.YP.rotationDegrees(180)); break;
            case SOUTH: break;
            case EAST:  poseStack.mulPose(Axis.YP.rotationDegrees(90)); break;
            case WEST:  poseStack.mulPose(Axis.YP.rotationDegrees(-90)); break;
            case UP:    poseStack.mulPose(Axis.XP.rotationDegrees(-90)); break;
            case DOWN:  poseStack.mulPose(Axis.XP.rotationDegrees(90)); break;
        }
        ...
        for (int i = 0; i < particleCount; i++) {
            ...
            // ランダム回転
            poseStack.mulPose(Axis.XP.rotationDegrees((animTime + i * 10) * 5.0f)); // 回転も速く
            poseStack.mulPose(Axis.YP.rotationDegrees((animTime + i * 20) * 5.0f));
