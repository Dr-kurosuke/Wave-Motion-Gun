...
public class TriggerUnitScreen extends VirtualScaledScreen<TriggerUnitMenu> {
    ...
    private void updateTurnDelta() {
        ...
        float yaw = com.example.wave_motion_gun.compat.VSCompat
                .shipYawAt(this.minecraft.level, menu.getBlockEntity().getBlockPos());
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        if (uiOpenProgress > 0.0f && startScissor(70, 0, 360, 200)) {
            ...
            if (!Float.isNaN(this.turnDelta)) {
                gauges.renderTurnIndicator(guiGraphics, this.turnDelta, TURN_LIMIT_DEGREES);
            }
            endScissor();
        }
