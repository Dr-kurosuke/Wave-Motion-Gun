...
public class MonitoringUnitScreen extends VirtualScaledScreen<MonitoringUnitMenu> {
    ...
    protected void init() {
        ...
        // Freq
        this.frequencyBox = new EditBox(this.font, 40, topY, 50, 20, new TextComponent("Freq"));
        ...
        this.addRenderableWidget(new Button(95, topY, 30, 20, new TextComponent("SET"), btn -> {
        ...
        // Colors
        int colorX = 280;
        this.innerColorBox = new EditBox(this.font, colorX, topY, 60, 20, new TextComponent("Inner"));
        this.innerColorBox.setValue(innerHex);
        this.innerColorBox.setMaxLength(6);
        this.addRenderableWidget(this.innerColorBox);

        this.outerColorBox = new EditBox(this.font, colorX + 70, topY, 60, 20, new TextComponent("Outer"));
        ...
        this.addRenderableWidget(new Button(colorX + 135, topY, 30, 20, new TextComponent("SET"), btn -> {
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
        ...
        drawSteelBox(poseStack, 20, 10, 455, 45, true);
        drawScaledString(poseStack, "Frequency", 40, 15, COLOR_TEXT_WHITE, 0.8f);
        boolean hasAdjuster = menu.hasAdjuster();
        drawScaledString(poseStack, hasAdjuster ? "Beam Colors (Hex)" : "Colors (Req. Adjuster)", 300, 15, hasAdjuster ? COLOR_TEXT_WHITE : COLOR_TEXT_GRAY, 0.8f);

        // Bottom Frame (Sliders)
        drawSteelBox(poseStack, 20, 170, 455, 120, true);
        ...
        int p1x = 20;
        drawHoloPanel(poseStack, p1x, holoY, panelW, holoH, "STRUCTURE");
        int lineY = holoY + 20;
        ...
        drawScaledString(poseStack, cachedBarrelsStr, p1x + 10, lineY, COLOR_HOLO_TEXT, 1.0f);
        lineY += 12;
        int tCount = menu.getTachyonCount();
        if (tCount != cachedTachyonCount) { cachedTachyonCount = tCount; cachedTachyonStr = "Tachyon: " + tCount + "/2"; }
        drawScaledString(poseStack, cachedTachyonStr, p1x + 10, lineY, (tCount > 0) ? COLOR_TEXT_GREEN : COLOR_TEXT_GRAY, 1.0f);
        lineY += 12;
        ...
        drawScaledString(poseStack, cachedChamberStr, p1x + 10, lineY, hasChamber ? COLOR_TEXT_GREEN : COLOR_TEXT_GRAY, 1.0f);

        // Panel 2: Ballistics Status (Center)
        int p2x = p1x + panelW + panelGap;
        drawHoloPanel(poseStack, p2x, holoY, panelW, holoH, "BALLISTICS");
        lineY = holoY + 20;
        ...
        drawScaledString(poseStack, cachedRangeLimitStr, p2x + 10, lineY, COLOR_HOLO_TEXT, 1.0f);
        lineY += 12;
        ...
        drawScaledString(poseStack, cachedRadiusLimitStr, p2x + 10, lineY, COLOR_HOLO_TEXT, 1.0f);
        lineY += 12;
        ...
        drawScaledString(poseStack, cachedDmgLimitStr, p2x + 10, lineY, COLOR_HOLO_TEXT, 1.0f);

        // Panel 3: System Status (Right)
        int p3x = p2x + panelW + panelGap;
        drawHoloPanel(poseStack, p3x, holoY, panelW, holoH, "SYSTEM");
        lineY = holoY + 20;
        ...
        drawScaledString(poseStack, cachedAdjusterStr, p3x + 10, lineY, hasAdjuster ? COLOR_TEXT_GREEN : COLOR_TEXT_GRAY, 1.0f);
        lineY += 12;
        ...
        drawScaledString(poseStack, cachedModeStr, p3x + 10, lineY, destructive ? COLOR_TEXT_RED : COLOR_TEXT_GREEN, 1.0f);
        lineY += 12;
        ...
        drawScaledString(poseStack, cachedSafetyStr, p3x + 10, lineY, safety ? COLOR_TEXT_YELLOW : COLOR_TEXT_GREEN, 1.0f);
        ...
        drawEnergyBar(poseStack, barX, barY, barTotalW, barH);
        ...
        drawScaledCenteredStringNoShadow(poseStack, cachedEnergyStr, VIRTUAL_WIDTH / 2, barY + 6, COLOR_TEXT_WHITE, 1.0f);

        // --- 4. Render Widgets ---
        // Labels for bottom switches
        drawScaledString(poseStack, "Destructive Mode", 95, 266, COLOR_TEXT_WHITE, 1.0f);
        if (!menu.hasChamber()) drawScaledString(poseStack, "(Req. Chamber)", 95, 276, COLOR_TEXT_RED, 0.7f);

        drawScaledString(poseStack, "Safety Release", 330, 266, COLOR_TEXT_WHITE, 1.0f);
    ...
    private void drawEnergyBar(PoseStack poseStack, int x, int y, int w, int h) {
        // Draw Background Strip
        fill(poseStack, x, y, x + w, y + h, 0xFF111111);
        ...
        for (int i = 0; i < segmentCount; i++) {
            ...
            fill(poseStack, (int)sx, y + 2, (int)ex, y + h - 2, color);
        }
    ...
    private void drawHoloPanel(PoseStack poseStack, int x, int y, int w, int h, String title) {
        ...
        // Frame
        fill(poseStack, x, y, x + w, y + 1, COLOR_HOLO_FRAME); // Top
        fill(poseStack, x, y + h - 1, x + w, y + h, COLOR_HOLO_FRAME); // Bottom
        fill(poseStack, x, y, x + 1, y + h, COLOR_HOLO_FRAME); // Left
        fill(poseStack, x + w - 1, y, x + w, y + h, COLOR_HOLO_FRAME); // Right

        // Title Box
        fill(poseStack, x, y, x + w, y + 12, COLOR_HOLO_FRAME);
        drawScaledCenteredStringNoShadow(poseStack, title, x + w / 2, y + 2, 0xFF000033, 0.9f);
    }
    ...
    @Override protected void renderBg(PoseStack p, float pt, int x, int y) {}
    @Override protected void renderLabels(PoseStack p, int x, int y) {}
}
