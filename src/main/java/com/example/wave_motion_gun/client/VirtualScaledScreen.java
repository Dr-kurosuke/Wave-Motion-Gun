...
public abstract class VirtualScaledScreen<T extends AbstractContainerMenu> extends AbstractContainerScreen<T> {
    ...
    // --- Scaled Text Helpers ---
    protected void drawScaledString(GuiGraphics guiGraphics, String text, float x, float y, int color) {
        drawScaledString(guiGraphics, text, x, y, color, 1.0f);
    }

    protected void drawScaledString(GuiGraphics guiGraphics, String text, float x, float y, int color, float scale) {
    ...
    /** 影付き・中央揃え */
    protected void drawScaledCenteredString(GuiGraphics guiGraphics, String text, float x, float y, int color) {
        drawScaledCenteredString(guiGraphics, text, x, y, color, 1.0f);
    }

    protected void drawScaledCenteredString(GuiGraphics guiGraphics, String text, float x, float y, int color, float scale) {
    ...
    /** 影なし・中央揃え */
    protected void drawScaledCenteredStringNoShadow(GuiGraphics guiGraphics, String text, float x, float y, int color) {
        drawScaledCenteredStringNoShadow(guiGraphics, text, x, y, color, 1.0f);
    }

    protected void drawScaledCenteredStringNoShadow(GuiGraphics guiGraphics, String text, float x, float y, int color, float scale) {
        int w = this.font.width(text);
        PoseStack poseStack = guiGraphics.pose();
        poseStack.pushPose();
        poseStack.translate(x - (w / 2.0f) * scale, y, 0);
     ...
     */
    protected void drawSteelBox(GuiGraphics guiGraphics, int x, int y, int w, int h, boolean rightShadow) {
    ...
    protected void drawHoloBox(GuiGraphics guiGraphics, int x, int y, int w, int h) {
        guiGraphics.fill(x, y, x + w, y + h, COLOR_HOLO_BG);
    }

    protected void drawLamp(GuiGraphics guiGraphics, int cx, int cy, boolean active) {
        int color = active ? 0xFF00FF00 : 0xFFFF0000;
        int r = 4;
        guiGraphics.fill(cx - r - 1, cy - r - 1, cx + r + 1, cy + r + 1, 0xFF888888);
        guiGraphics.fill(cx - r, cy - r, cx + r, cy + r, color);
        guiGraphics.fill(cx - 1, cy - 2, cx + 2, cy - 1, 0xCCFFFFFF);
    }
}
