...
public class ShockCannonScreen extends AbstractContainerScreen<ShockCannonMenu> {
    ...
    protected void init() {
        ...
        int fireBtnX = x + 68;
        int fireBtnY = y + 33;
        this.addRenderableWidget(new FireButton(fireBtnX, fireBtnY, 40, 22, Component.literal("FIRE"), 1.0f, btn -> {
        ...
        // 入力ボックス (数値のみ)
        this.delayBox = new EditBox(this.font, delayX, delayY, 30, delayH, Component.literal("Delay"));
        ...
        this.addRenderableWidget(new SmallButton(delayX + 32, delayY, 29, delayH, Component.literal("save"), btn -> {
            try {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredScaledString(guiGraphics, "G", swStartX + centerOffset, labelY, 0xFF55FF55, 0.8f);
        drawCenteredScaledString(guiGraphics, "R", swStartX + gap + centerOffset, labelY, 0xFFFF5555, 0.8f);
        drawCenteredScaledString(guiGraphics, "B", swStartX + gap * 2 + centerOffset, labelY, 0xFF5555FF, 0.8f);

        // Delayラベル
        drawCenteredScaledString(guiGraphics, "Delay(tick)", this.leftPos + 83, this.topPos + 56, 0xFFCCCCCC, 0.7f);
    }

    @Override
    protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int mouseX, int mouseY) {
        drawSteelBox(guiGraphics, this.leftPos, this.topPos, this.imageWidth, this.imageHeight);

        int sx = this.leftPos + 24;
        int sy = this.topPos + 34;
        guiGraphics.fill(sx, sy, sx + 18, sy + 18, 0xFF222222);
        guiGraphics.fill(sx, sy, sx + 17, sy + 1, 0xFF333333);
        guiGraphics.fill(sx, sy, sx + 1, sy + 17, 0xFF333333);

        int pInvY = this.topPos + 83;
        guiGraphics.fill(this.leftPos + 7, pInvY, this.leftPos + 169, pInvY + 76, 0xFF444444);
    }

    private void drawSteelBox(GuiGraphics guiGraphics, int x, int y, int w, int h) {
        guiGraphics.fill(x, y, x + w, y + h, COLOR_STEEL_BG);
        guiGraphics.fill(x, y, x + w, y + 2, COLOR_STEEL_LIGHT);
        guiGraphics.fill(x, y, x + 2, y + h, COLOR_STEEL_LIGHT);
        guiGraphics.fill(x, y + h - 2, x + w, y + h, COLOR_STEEL_DARK);
        guiGraphics.fill(x + w - 2, y, x + w, y + h, COLOR_STEEL_DARK);
    }

    private void drawCenteredScaledString(GuiGraphics guiGraphics, String text, float x, float y, int color, float scale) {
        int w = this.font.width(text);
        PoseStack poseStack = guiGraphics.pose();
        poseStack.pushPose();
        poseStack.translate(x - (w / 2.0f) * scale, y, 0);
    ...
    private class SmallButton extends Button {
        ...
        public void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
            ...
            // 左上
            guiGraphics.blit(WIDGETS_LOCATION, this.getX(), this.getY(), halfWidth, topHeight, 0, texY, halfWidth, topHeight, 256, 256);
            // 右上
            guiGraphics.blit(WIDGETS_LOCATION, this.getX() + halfWidth, this.getY(), this.width - halfWidth, topHeight, 200 - (this.width - halfWidth), texY, this.width - halfWidth, topHeight, 256, 256);

            // 左下 (テクスチャの下端から描画することで枠線を維持)
            guiGraphics.blit(WIDGETS_LOCATION, this.getX(), this.getY() + topHeight, halfWidth, bottomHeight, 0, texY + 20 - bottomHeight, halfWidth, bottomHeight, 256, 256);
            // 右下
            guiGraphics.blit(WIDGETS_LOCATION, this.getX() + halfWidth, this.getY() + topHeight, this.width - halfWidth, bottomHeight, 200 - (this.width - halfWidth), texY + 20 - bottomHeight, this.width - halfWidth, bottomHeight, 256, 256);

            int textColor = this.active ? COLOR_TEXT_WHITE : 0xFFAAAAAA;
            guiGraphics.drawCenteredString(minecraft.font, this.getMessage(), this.getX() + this.width / 2, this.getY() + (this.height - 8) / 2, textColor);
