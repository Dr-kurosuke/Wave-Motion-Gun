...
public class AiConfigScreen extends Screen {
    ...
    public AiConfigScreen(@Nullable Screen parent) {
        super(new TranslatableComponent("gui.wave_motion_gun_mod.ai_config.title"));
        this.parent = parent;
    ...
    protected void init() {
        ...
        // AIモード ON/OFF
        this.addRenderableWidget(new Button(centerX - 100, 35, 200, 20, modeLabel(), btn -> {
            this.aiMode = !this.aiMode;
            btn.setMessage(modeLabel());
        }));

        // プロバイダ切替
        this.addRenderableWidget(new Button(centerX - 100, 60, 200, 20, providerLabel(), btn -> {
            captureFields();
        ...
        this.apiKeyField = new EditBox(this.font, centerX - 100, 98, 176, 20,
                new TranslatableComponent("gui.wave_motion_gun_mod.ai_config.api_key"));
        ...
        // マスク解除ボタン(*: マスク中 / A: 表示中)
        this.addRenderableWidget(new Button(centerX + 80, 98, 20, 20,
                new TextComponent(revealKey ? "A" : "*"), btn -> {
            this.revealKey = !this.revealKey;
            btn.setMessage(new TextComponent(this.revealKey ? "A" : "*"));
        }));

        // モデルID
        this.modelField = new EditBox(this.font, centerX - 100, 138, 200, 20,
                new TranslatableComponent("gui.wave_motion_gun_mod.ai_config.model"));
        ...
        if (isOpenAi()) {
            this.baseUrlField = new EditBox(this.font, centerX - 100, 178, 200, 20,
                    new TranslatableComponent("gui.wave_motion_gun_mod.ai_config.base_url"));
        ...
        // 完了(保存)
        this.addRenderableWidget(new Button(centerX - 100, this.height - 28, 200, 20,
                CommonComponents.GUI_DONE, btn -> this.onClose()));
    }

    private TranslatableComponent modeLabel() {
        return new TranslatableComponent("gui.wave_motion_gun_mod.ai_config.mode",
                I18n.get(aiMode ? "options.on" : "options.off"));
    }

    private TranslatableComponent providerLabel() {
        ...
        return new TranslatableComponent("gui.wave_motion_gun_mod.ai_config.provider", name);
    }
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        drawCenteredString(poseStack, this.font, this.title.getString(), centerX, 15, 0xFFFFFF);

        // ラベル(各入力欄の直上)
        drawString(poseStack, this.font, I18n.get("gui.wave_motion_gun_mod.ai_config.api_key"), centerX - 100, 88, 0xAAAAAA);
        drawString(poseStack, this.font, I18n.get("gui.wave_motion_gun_mod.ai_config.model"), centerX - 100, 128, 0xAAAAAA);
        if (isOpenAi()) {
            drawString(poseStack, this.font, I18n.get("gui.wave_motion_gun_mod.ai_config.base_url"), centerX - 100, 168, 0xAAAAAA);
        }

        // 注意書き(画面が狭い場合は省略)
        if (this.height >= 270) {
            drawCenteredString(poseStack, this.font, I18n.get("gui.wave_motion_gun_mod.ai_config.note1"), centerX, this.height - 64, 0x777777);
            drawCenteredString(poseStack, this.font, I18n.get("gui.wave_motion_gun_mod.ai_config.note2"), centerX, this.height - 52, 0x777777);
        }
