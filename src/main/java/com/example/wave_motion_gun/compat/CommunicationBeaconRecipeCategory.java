...
public class CommunicationBeaconRecipeCategory implements IRecipeCategory<ItemStack> {
    ...
    public CommunicationBeaconRecipeCategory(IGuiHelper guiHelper) {
        ...
        this.title = new TranslatableComponent("jei.wave_motion_gun_mod.communication_beacon.title");
    }

    @Override
    @SuppressWarnings("removal")
    public ResourceLocation getUid() {
        return new ResourceLocation(ExampleMod.MODID, "communication_beacon");
    }
    ...
    @Override
    public void draw(ItemStack recipe, IRecipeSlotsView recipeSlotsView, PoseStack poseStack, double mouseX, double mouseY) {
        Minecraft mc = Minecraft.getInstance();

        // 左側の "?" マーク
        poseStack.pushPose();
        poseStack.scale(2.5f, 2.5f, 1.0f);
        mc.font.draw(poseStack, "?", 6, 5, 0xFF000000);
        poseStack.popPose();

        // 中央の矢印とメッセージ
        mc.font.draw(poseStack, ">>>", 60, 20, 0xFF404040);
        mc.font.draw(poseStack, new TranslatableComponent("jei.wave_motion_gun_mod.communication_beacon.communicate"), 40, 5, 0xFF555555);
        ...
        switch (rank) {
            case "S":
                displayText = new TranslatableComponent("jei.wave_motion_gun_mod.communication_beacon.clearance_s");
                color = 0xFFFFAA00; // Gold
                break;
            case "A":
                displayText = new TranslatableComponent("jei.wave_motion_gun_mod.communication_beacon.clearance_a");
                color = 0xFF55FFFF; // Aqua
                break;
            case "B":
                displayText = new TranslatableComponent("jei.wave_motion_gun_mod.communication_beacon.clearance_b");
                color = 0xFFFFFF55; // Yellow
                break;
            case "C":
                displayText = new TranslatableComponent("jei.wave_motion_gun_mod.communication_beacon.clearance_c");
                color = 0xFFFFFFFF; // White
                break;
            case "D":
                displayText = new TranslatableComponent("jei.wave_motion_gun_mod.communication_beacon.clearance_d");
                color = 0xFFAAAAAA; // Gray
                break;
            default:
                displayText = new TranslatableComponent("jei.wave_motion_gun_mod.communication_beacon.clearance_unknown");
        ...
        mc.font.draw(poseStack, displayText, 5, 60, color);
        poseStack.popPose();
    }
